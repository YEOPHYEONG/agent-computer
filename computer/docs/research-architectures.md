...
Native runtime subagents are optional and permission-bound. Codex native subagents require an explicit user request for subagents, delegation, or parallel agent work. Claude Code project subagents can be materialized from the same specs only after explicit approval. Without native runtime support, the worker packets preserve the same specialist structure.
