...
Agent Computer should not silently spawn runtime subagents. If the user explicitly asks for subagents, delegation, or parallel agent work and the runtime supports it, Wide Mode may use them. Otherwise, process worker packets sequentially while preserving the same schema.
