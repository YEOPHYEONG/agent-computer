...
function subagentTraceIntegrityIssues(text, projectResearch = [], filePath = '') {
  ...
  if (composerResult && !isFilledReportComposerResult(composerResult.text)) {
    issues.push(issue('important', 'research/subagent-results/ac-report-composer.md exists but does not contain a complete report-composition trace.'));
  }
  ...
  for (const [name, pattern] of promisedArtifacts) {
    ...
    if (isReportOutput && subagentResultFiles.length && promised && !present) {
      issues.push(issue('important', `Subagent-backed report orchestration references research/${name}.md, but that artifact is missing.`));
    }
  }
...
function isFilledReportComposerResult(text) {
  ...
  const required = [
    /Questions That Changed The Recommendation/i,
    /Claim[-\s]*Evidence[-\s]*Confidence[-\s]*Caveat/i,
    /Assumptions/i,
    /Validate Later|Validation Later/i,
    /Downstream Handoff|Handoff/i
  ];
