...
`;
  ...
  await writeText(out, body);
  const files = [contractFile, architectureDecisionFile, sourceMapFile, evidenceStoreFile, questionLedgerFile, claimMapFile, hypothesisMapFile, redTeamCritiqueFile, synthesisPlanFile, subagentRuntime.planFile, retryLogFile, researchQaFile, ...subagentRuntime.files.filter((file) => file !== subagentRuntime.planFile)];
...
| Required artifacts | research-contract.md, architecture-decision.md, runtime-subagent-plan.md, subagent-orchestration.md, subagent-results/README.md, subagent-results/_template.md, worker-packets/ac-intent-analyst.md, worker-packets/ac-research-architect.md, worker-packets/ac-*.md, source-map.md, evidence-store.md, question-ledger.md, hypothesis-map.md, red-team-critique.md, synthesis-plan.md |
...
function renderSynthesisPlan(architecture, question, mode, claimReviews) {
  ...
  return [
    ...
    `- Downstream preservation rule: report-writer and ppt-builder should preserve the selected architecture instead of flattening the research into generic bullets.`
  ].join('\n');
}
...
function renderDownstreamHandoff(mode, architecture, topic, claimReviews) {
  ...
  return [
    `- Recommended next agent: report-writer for a decision-ready report; ppt-builder if the user asks for a deck; email-operator only after claim-safety review.`,
    ...
    `- Source/reference layer: preserve source IDs and claim statuses in report appendices, speaker notes, or PPT reference slides.`
  ].join('\n');
}
