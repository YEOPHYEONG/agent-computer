...
function executionGateDecision(request, chain, intent, routing, researchMode, researchArchitecture) {
  ...
  const materialChain = chain.some((agent) => ['deep-dive-researcher', 'ppt-builder', 'agent-builder', 'friend-counselor'].includes(agent));
...
function researchModeDecision(request, chain) {
  ...
  if (broadCoverage && synthesis) {
    return {
      ...
      artifacts: ['research-contract.md', 'architecture-decision.md', 'runtime-subagent-plan.md', 'subagent-orchestration.md', 'subagent-results/', 'worker-packets/', 'source-map.md', 'evidence-store.md', 'hypothesis-map.md', 'red-team-critique.md', 'synthesis-plan.md', 'claim-verification-map.md', 'question-ledger.md']
    };
  }
...
function rolesForRoutedArchitecture(architecture, mode) {
  const map = {
    'Strategic Decision Research': ['intent-analyst', 'research-architect', 'source-scout', 'evidence-verifier', 'mechanism-analyst', 'red-team-critic', 'synthesis-architect'],
    'Product / GTM Strategy Research': ['intent-analyst', 'research-architect', 'source-scout', 'market-mapper', 'gtm-strategist', 'evidence-verifier', 'red-team-critic', 'synthesis-architect'],
    'Growth Case Reconstruction': ['intent-analyst', 'research-architect', 'source-scout', 'case-benchmark-worker', 'mechanism-analyst', 'evidence-verifier', 'red-team-critic', 'synthesis-architect'],
    'Market / Competitive Landscape': ['intent-analyst', 'research-architect', 'source-scout', 'market-mapper', 'evidence-verifier', 'red-team-critic', 'synthesis-architect'],
    'Technical / Implementation Research': ['intent-analyst', 'research-architect', 'technical-docs-reader', 'evidence-verifier', 'red-team-critic', 'synthesis-architect'],
    'Source-Heavy Evidence Review': ['intent-analyst', 'research-architect', 'source-scout', 'evidence-verifier', 'red-team-critic', 'synthesis-architect'],
    'Wide Benchmark To Deep Synthesis': ['intent-analyst', 'research-architect', 'source-scout', 'case-benchmark-worker', 'evidence-verifier', 'mechanism-analyst', 'red-team-critic', 'synthesis-architect']
  };
  const base = map[architecture] || map['Strategic Decision Research'];
  if (mode === 'Wide' || mode === 'Hybrid') return [...new Set([...base, 'case-benchmark-worker'])];
  return base;
}
